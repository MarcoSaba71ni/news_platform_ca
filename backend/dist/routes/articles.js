...
router.post("/", authenticateToken, validateCreateArticle, async (req, res) => {
    try {
        ...
        const [result] = await pool.execute(`INSERT INTO articles (title, body, category, media_url, media_alt, user_id)
       VALUES (?, ?, ?, ?, ?, ?)`, [title, body, category, media_url, media_alt, user_id]);
        const article = {
            id: result.insertId,
            title,
            body,
